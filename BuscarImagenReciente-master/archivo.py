...
def obtener_imagen_mas_reciente(carpeta_imagenes):
    ...
    print(f"Lista de imágenes: {lista_imagenes}")
    ...
    print(f"Imágenes filtradas: {lista_imagenes}")
    ...
    print(f"Imágenes modificadas hoy: {lista_imagenes}")
    ...
    print(f"Imagen más reciente: {imagen_mas_reciente}")
    
    return imagen_mas_reciente

@app.route('/imagen_reciente', methods=['GET'])
def imagen_reciente():
    print("Ruta /imagen_reciente llamada")
    carpeta_imagenes = '//NASNevado/Public/Naranjal1'
    imagen = obtener_imagen_mas_reciente(carpeta_imagenes)
    if imagen:
        return jsonify({'imagen_mas_reciente': imagen})
    else:
        return jsonify({'mensaje': 'No se encontraron imágenes en la carpeta.'}), 404

if __name__ == '__main__':
    print("Iniciando servidor Flask...")
    app.run(host='0.0.0.0', port=5001)  # Cambiar el puerto a 5001